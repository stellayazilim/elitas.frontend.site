...
{#each products as product}
  ...
  {#if filtered && product.items.filter(item => item.name.toLowerCase().includes(filtered && filtered.toLowerCase() || '')).length === 0}
    <div class="col-12 m-1" >
      <span class="span">Aradığınız ürün bu kategoride yok</span>
    </div>
  {:else}

  {#each product.items as item}
    {#if !filtered || (filtered && item.name.toLowerCase().includes(filtered && filtered.toLowerCase() || ''))}
      <div class="col-lg-4 col-md-6 col-sm-12 pb-1">
        <div class="card product-item border-0 mb-4">
          <div class="card-header product-img position-relative overflow-hidden bg-transparent border height-contain p-0">
          ...
          <div class="card-footer d-flex justify-content-between bg-light border">
            <a href="/product/{product.slug}/{item.slug}#_navbar" class="btn btn-sm text-dark p-0 w-100">
              <i class="fas fa-eye text-info mr-1"></i>Detayları Gör
            </a>
          </div>
        </div>
      </div>
